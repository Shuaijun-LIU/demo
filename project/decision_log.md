...
- 2026-08-12 — 首次 Pages 实载探针定位到 Franka 原始 67 个模型依赖并发中出现 `ERR_HTTP2_PROTOCOL_ERROR`，导致 XML 已返回但 MuJoCo 无法进入 ready。Web Panda 改为按刚体无损合并外观 OBJ 并编码为 10 个完整 MSH 网格（约 2.4 MB），模型依赖降至 22 个；不减面，不改变关节、惯量或碰撞网格。
- 2026-08-13 — 按用户反馈删除全局标题区，场景选择器直接成为页面首个主体元素；六场景共享低饱和双蓝棋盘格与细白网格线。棋盘仅属于 Web 渲染层，MJCF 隐藏物理平面与接触验证保持不变。
- 2026-08-13 — 按用户反馈进一步压深三维主体区域：视口背景/雾使用 `#d2d5d4`，棋盘使用 `#456c8e / #7193ae`；页面 UI、白线、模型灯光、相机和物理定义保持不变，避免整体降灯导致 Franka 细节丢失。
- 2026-08-13 — Demo03 悬空根因是称重显示器与称台间缺少 95 mm 立柱；Demo06 悬空根因是去核头横梁缺少连接底座的门架立柱。采用补齐真实支撑链，不下压任务物体。主体背景进一步改为近黑炭灰 `#252a2e`，白色页面 UI、灯光和棋盘配色不变。
